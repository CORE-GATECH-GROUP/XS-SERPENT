...
Notes:
    ...
    Branch syntax is similar to the nominal condition syntax, where 
    perturbed conditions for each material are given on a line. ::
    ...
    Two branches are defined here.  The first, named hUO2 indicates 
    that the temperature of material 'UO2' should be changed to 1200K. 
    The second, hFuel, changes the temperature for four materials to 1200K
    ...
    The nominal burnup schedule, if given, will be the first line with 
    'burn' as the first word. 
...
def readparam(infile: str, args=None):
    ...
    :return: 
        ...
        branch_d - dictionary of branch names with perturbed parameters as values
            If a perturbed branch has burnup, then a key ``'burn': <burnstring>`` is added, 
            where ``<burnstring>`` indicates the freeform burnup schedule
        exe_str - Formatable string to execute ``SERPENT`` runs. The format argument
            will be the filename. ``./sss2 {0} > {0}.o``
    ...
    if not os.path.exists(infile):
        xsboa.messages.warn('File {} does not exist and cannot be processed'.format(os.path.join(os.getcwd(), infile)),
                            'readparam()', args)
        return (False, -1), nom_d, branch_d, exe_str, var_l
            ...
            if '/*' in line and 'xsboa' in line:
                ...
                while 'xsboa' not in line and '*/' not in line:
                    ...
                    if line.strip() != '':
                        ...
                        if lsplit[0] == 'nom':
                            ...
                            while line.strip() != '' and line.split()[0] not in (
                                ...
                                lsplit = line.split()
                                if lsplit[0] == 'burn':
                                    branch_d[branch_name]['burn'] = line.strip()
                                else:
                    ...
                    if not b_flag:
                        ...
                        lcount += 1
                    if line == '':
                        return (block_start, block_end), nom_d, branch_d, exe_str, var_l
                block_end = lcount
                xsboa.messages.status('  done', args)
                return (block_start, block_end), nom_d, branch_d, exe_str, var_l
            line = inobj.readline()
            lcount += 1
        return (True, -2), nom_d, branch_d, exe_str, var_l


# testing
if __name__ == '__main__':
    locs, nom, branches, exe, var_lst = readparam(os.path.join('testing', 'uo2mox.txt'), None)
    print(locs)
    print('--')
    for each in nom:
        print(each, nom[each])
